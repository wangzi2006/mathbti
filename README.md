...
每个答案为五个坐标增加或减少分数，权重可以是小数。完成后，每个维度以 0 为中心，负方向按该玩家已回答题目的可达最小值归一化到 -1～0，正方向按可达最大值归一化到 0～1，再计算与普通结果目标坐标的欧氏距离。少数结果可以设置轻微的距离校准值，用于调整过度悬殊的出现比例。结果页使用温和的非线性折算将内部坐标显示为 0～100%，但结果匹配仍使用未经折算的内部坐标。若出现完全并列，则随机选择一个结果。被跳过的题不参与坐标计算。
